...
def init_retinaface_model(model_name, half=False, device='cuda'):
    ...
    model_path = load_file_from_url(url=model_url, model_dir='weights/CodeFormer.facelib', progress=True, file_name=None)
...
def init_yolov5face_model(model_name, device='cuda'):
    if model_name == 'YOLOv5l':
        model = YoloDetector(config_name='CodeFormer.facelib/detection/yolov5face/models/yolov5l.yaml', device=device)
        model_url = 'https://github.com/sczhou/CodeFormer/releases/download/v0.1.0/yolov5l-face.pth'
    elif model_name == 'YOLOv5n':
        model = YoloDetector(config_name='CodeFormer.facelib/detection/yolov5face/models/yolov5n.yaml', device=device)
...
#     model_path = os.path.join('weights/CodeFormer.facelib', list(f_id.keys())[0])
#     if not os.path.exists(model_path):
#         download_pretrained_models(file_ids=f_id, save_path_root='weights/CodeFormer.facelib')
